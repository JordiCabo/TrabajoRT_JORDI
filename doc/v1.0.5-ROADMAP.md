# Roadmap v1.0.5 - Corrección de Debilidades Identificadas

**Versión**: 1.0.5 (Planificado)  
...
Corregir las debilidades identificadas en [doc/ASSESSMENT.md](ASSESSMENT.md) que aún persisten tras completar v1.0.4:

1. **Falta de control de errores en threading**
2. **Ausencia de logging/diagnósticos**
3. **Configuración dispersa de frecuencias**
4. **Mutex único compartido (posible contención)**
5. **Scheduling no configurado (jitter alto)**
...
// ⚠️ No se verifica retorno (int retval)
```

**Solución Propuesta (v1.0.5)**:
```cpp
...
### 2. Logging Básico para Diagnósticos

**Problema Actual**:
- No hay trazas de ejecución
- Difícil diagnosticar qué hilo falló
- Sin timestamp para análisis de jitter
...
### 3. Configuración Centralizada de Frecuencias
...
- Compilación constexpr (sin overhead runtime)
...
**Archivos**:
- [ ] Crear `include/VariablesCompartidas.h` (nuevo diseño con separación de mutex)
- [ ] Actualizar `src/VariablesCompartidas.cpp`
- [ ] Actualizar todos los `Hilo*.cpp` para usar nuevo esquema

---

### 5. Configuración de Scheduler FIFO/RR

**Problema Actual**:
- Scheduling por defecto (SCHED_OTHER) tiene jitter alto
- Depende de carga del sistema
- No determinístico en tiempo real

**Solución Propuesta (v1.0.5)**:
Agregar función en `include/ThreadUtils.h`:
...
public:
     ...
     * @return 0 si éxito, -1 si falla (requiere privilegios)
     */
...
- [ ] Documento: `doc/REALTIME-SCHEDULER.md` (guía)

---

## 📊 Tabla de Implementación v1.0.5

| Mejora | Archivos | Complejidad | Impacto | Estimado |
|--------|----------|-------------|--------|----------|
| 1. Error Handling | 8 Hilo*.h/cpp | ⭐ Baja | Alto | 2h |
| 2. Logging | Logger.h/cpp + Hilo*.cpp | ⭐⭐ Media | Alto | 3h |
| 3. Config Centralizada | Config.h + test/testHilo.cpp | ⭐ Baja | Alto | 1h |
| 4. Mutex Separados | VariablesCompartidas.h/cpp | ⭐⭐⭐ Alta | Medio | 4h |
| 5. Real-time Scheduler | ThreadUtils.h/cpp + docs | ⭐⭐ Media | Medio | 2h |
...
- [ ] **Fase 4: Mutex Separados** (4h)
  - [ ] Diseño de nueva VariablesCompartidas
  - [ ] Implementación
...
1. **doc/LOGGING.md**: Guía de uso del sistema de logging
2. **doc/CONFIG.md**: Explicación de parámetros centralizados
3. **doc/REALTIME-SCHEDULER.md**: Guía de configuración FIFO/RR
...
- [doc/ASSESSMENT.md](ASSESSMENT.md) - Evaluación completa
- [doc/CHANGELOG.md](CHANGELOG.md) - Historial de versiones
- [include/Hilo.h](../include/Hilo.h) - Patrón actual de threading
