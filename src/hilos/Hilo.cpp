...
Hilo::Hilo(std::shared_ptr<DiscreteSystem> system, 
    ...
    frequency_(frequency), name_(log_prefix), system_raw_(nullptr), input_raw_(nullptr), output_raw_(nullptr),
...
Hilo::Hilo(DiscreteSystem* system, double* input, double* output, bool* running,
    ...
    frequency_(frequency), name_(log_prefix), system_raw_(system), input_raw_(input), output_raw_(output),
 ...
 */
void Hilo::run() {
    // Bloquear SIGINT/SIGTERM para que solo HiloIntArranque los maneje
    bloquear_signals();
...
/**
 * @brief Bloquea SIGINT y SIGTERM en el hilo actual
 * 
 * Crea un sigset con SIGINT y SIGTERM bloqueados y lo aplica
 * al hilo actual con pthread_sigmask().
 */
...
/**
 * @brief Desbloquea SIGINT y SIGTERM en el hilo actual
 * 
 * Crea un sigset con SIGINT y SIGTERM desbloqueados y lo aplica
 * al hilo actual con pthread_sigmask().
 */
